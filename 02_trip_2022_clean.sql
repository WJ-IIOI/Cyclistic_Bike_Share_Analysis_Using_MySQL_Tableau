...
CREATE TABLE IF NOT EXISTS trip_2022_clean AS (SELECT ride_id,
    rideable_type,
    started_at,
    ended_at,
...
SET 
    bike_type = TRIM('_bike' FROM bike_type)
;
...
UPDATE trip_2022_clean 
SET 
    ride_type = CONCAT(UPPER(SUBSTRING(ride_type, 1, 1)),
            LOWER(SUBSTRING(ride_type, 2)))
;
